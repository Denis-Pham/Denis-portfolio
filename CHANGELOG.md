...
**Nội dung:**
1. **Đính chính teamwork KPI System (lời Denis):** hết "solo/một mình/Sole designer & builder" — sự thật: Denis thiết kế khung KPI, công thức rà soát cùng **BOM/BOD**, phối hợp **BA team tech** tự động hóa tính toán & ghi nhận, riêng dashboard Denis tự xây bằng Claude Code. Sửa: evidence.role (featured), bullet Hasaki 5, case-study (meta Role, meta **Team — HẾT "Pending confirmation/Chờ bổ sung"**, summary, role bullets, constraints 1 & 3).
2. **"real-time/thời gian thực" → "daily/hằng ngày" toàn site** (Denis xác nhận: dữ liệu cập nhật theo NGÀY; trước đây tính vào đầu tháng, mất 5 ngày): hero stat 2 (value "live"→"daily" + label + UI_I18N mới cho `.unit`), loop card bước 3, bullet Hasaki 6, featured summary + metric 1, case study (subtitle/summary/context/challenge/decision 2/before-after "Daily"), meta + og:description index (fix luôn "realtime" sót từ đợt 13/06) + meta description case-study.html.
3. **Số liệu quy mô mới từ Denis:** Allied thêm "Managed **~20** QC inspectors"; SIPHER thêm "đội **~10 người** đồng quản lý cùng lead art" (FPT QA 2 người — Denis bảo bỏ qua). Contact reason 3 thêm "or project-delivery role" (role ranking: Ops chính, PM phụ, BA không nhắm).
4. **Nhóm side project + reorder grid:** thứ tự mới Task WF (lead) → Ops WF (wide) → Sales×KPI → Bills → Hermes → Chi Tiêu (MỚI) → Vivi Soul; 3 card cuối gắn badge "Side project / Dự án cá nhân" (field `badge` + CSS `.project-badge`). **Hermes viết lại thành trợ lý CÁ NHÂN** (Denis: "không liên quan tới công việc") + câu authorship rõ ("tôi kiểm tra và duyệt bản cuối").
5. **Card MỚI "Chi Tiêu — Family Finance Hub"** (side project; facts từ chitieu-dashboard/CHANGELOG: Google Sheets + Apps Script, đóng gói 1 file HTML, budget cycle + thực đơn + đi chợ + việc nhà) + vẽ `projects/chitieu-hub.svg` theo phong cách bộ SVG cũ (palette xanh CT). Lưu ý: launch.json còn entry "denis-finance" trỏ `D:\claude agent\` đã KHÔNG tồn tại — "trang mới về tài chính" Denis nhắc chính là chitieu-dashboard.
6. **4 card nội bộ (Task WF, Ops WF, Sales×KPI, Bills) thêm note** "Internal company system — illustrated with mockups / Hệ thống nội bộ công ty — hình minh họa" (field `note` + CSS `.project-note`, flex:0 để không dính stretch của `.project-body p`) — trung thực về evidence vì Denis chưa duyệt screenshot thật. Ops Workflows ghi rõ "tách biệt với hệ thống thực thi task hằng ngày" + "xây từ con số 0, liên tục cải tiến" (lời Denis); Sales×KPI + Bills định vị lại là "board kiểm soát quanh hệ thống KPI" (lời Denis).
7. **Chrome khác:** thêm canonical link (index); footer thêm link CV (class `.cv-link`, UI_I18N swap href sang cv-vi.pdf khi VI; print ẨN link này — style.css); bump `?v=20260716` cả 2 trang; renderProjects hỗ trợ `badge`/`note`.
8. **Xóa 2 CV cũ** theo lệnh Denis ("bỏ đi") — trước đó đã verify: gitignored, chưa từng commit, 404 trên production; khôi phục được từ Drive trash trong 30 ngày.
**Verify:** preview DOM EN + VI đầy đủ (stat daily/hằng ngày, evidence role mới, order 7 card, 3 badge, 4 note, footer CV href swap, contact reason mới, **0** "real-time/thời gian thực/solo/một mình/Pending confirmation" trên cả 2 trang); case study đổi EN↔VI tại chỗ OK; mobile 375px không tràn ngang; dark mode token badge/note ăn đúng; console sạch; SVG mới fetch 200 + DOMParser sạch (naturalWidth=0 chỉ là throttle tab nền — bẫy cũ). In lại CẢ HAI CV (10 trang/bản; verify pypdf: EN full-text pass, VI pass qua ASCII probe — extract tiếng Việt có dấu từ PDF lỗi decode font subset, KHÔNG phải lỗi nội dung, DOM nguồn in đã verify; badge in ra chữ hoa "SIDE PROJECT" do text-transform).
**Lý do / ghi chú:** Denis xem preview và duyệt push cùng ngày 2026-07-16. Chưa làm og:image (cần ảnh raster, optional). Còn chờ Denis: có cho dùng screenshot thật đã che số liệu không (hiện dùng note nội bộ thay thế), tên bằng chính thức HCMUT trên bằng (đang giữ "Bachelor's degree").
